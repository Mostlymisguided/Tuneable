...
export default function PlaceProfileScreen() {
  ...
  const placeId =
    typeof placeIdParam === 'string' ? decodeURIComponent(placeIdParam) : '';
    ...
    async (isRefresh = false) => {
      ...
      try {
        ...
        if (place?.featureType === 'country') {
          ...
          setSubtitle(parts.length ? parts.join(', ') : null);
        }
        setTipTotal(data.stats?.globalPlaceAggregate ?? 0);
        ...
        setError(
          err instanceof Error ? err.message : 'Place not found or failed to load.'
        );
        if (!isRefresh) {
          setMedia([]);
          setRelatedPlaces([]);
      ...
      <View style={styles.mosaicWrap}>
        {mosaicCovers.length >= 4 ? (
          ...
          <View style={[styles.mosaicSingle, styles.mosaicFallback]}>
            <Ionicons name="location" size={42} color="#38bdf8" />
          </View>
        )}
      </View>
      ...
      {!loading && relatedPlaces.length > 0 ? (
        <View style={styles.chipRow}>
          {relatedPlaces.map((related) => {
            const href = getPlaceProfileHref(related.placeId);
            if (!href) return null;
            return (
              <Pressable
                key={related.placeId}
                ...
                <Text style={styles.placeChipText}>{related.name}</Text>
              </Pressable>
            );
          })}
        </View>
