...
const CreatorUpload: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // AI Usage fields (disclosure derived server-side)
      uploadData.append('aiUsed', aiUsage.used.toString());
      const cleanedTools = cleanAiTools(aiUsage.tools);
      if (cleanedTools.length > 0) {
        uploadData.append('aiTools', JSON.stringify(cleanedTools));
      }
      if (aiUsage.notes?.trim()) {
        uploadData.append('aiNotes', aiUsage.notes.trim());
      }
        ...
        <div className="card bg-black/40 backdrop-blur-sm border border-purple-500/20 rounded-lg p-8">
          ...
          <div className="space-y-6">
            ...
            <div className="border-t border-gray-600 pt-6">
              ...
              <AiToolsEditor value={aiUsage} onChange={setAiUsage} disabled={isUploading} />
            </div>
